<template>
    <div class="mapcanvas">
        ...
        <button class="whitefill-button" @click="whiteFill">白底</button>
        <button class="greyfill-button" @click="greyFill">灰底</button>
    </div>
</template>
...
<script setup>
    ...
    let fillcolor = ref('rgba(0,0,0)');
    const whiteFill = () => {
        fillcolor = ref('rgba(255, 255, 255)');
        const data = mapStore.geoJsonData;
        if (data) {
            const bounds = calculateBounds(data);
            setupCanvasSize(mapCanvas.value, bounds);
            drawRegions(data, bounds, fillcolor.value);
        }
    }
    const greyFill = () => {
        fillcolor = ref('rgba(128, 128, 128)');
        const data = mapStore.geoJsonData;
        if (data) {
            const bounds = calculateBounds(data);
            setupCanvasSize(mapCanvas.value, bounds);
        ...
        // ctx.fillStyle = fillcolor; // 设置填充颜色为白色
        // ctx.lineWidth = 5;
        // ctx.fillRect(0, 0, mapCanvas.value.width, mapCanvas.value.height); // 填充整个画布
    }
    // 初始化画布
    let ctx = null;
    ...
    watch(() => mapStore.geoJsonData, (newData) => {
        if (newData) {
            ...
            setupCanvasSize(mapCanvas.value, bounds);
            drawRegions(newData, bounds,fillcolor.value);
        }
    });
    ...
    // 绘制行政区域
    const drawRegions = (data, bounds,fillcolor) => {
